...
public class BlockedPageRankCalculateReduce extends MapReduceBase implements Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
        ...
        while (localResidual > 0.001 * PR.size()) {
        	...
        	while (BE_page_iter.hasNext()) {
        		...
        		for (String page_u : edges_in_block) {
        			pr = (NPR.get(page_u) > 0) ? NPR.get(page_u) : PR.get(page_u);
    	...
    	String highestRankNode = "";
    	PR_page_iter = PR.keySet().iterator();
    	while (PR_page_iter.hasNext()) {
    		String node_id = PR_page_iter.next().toString();
    		if (highestRank < PR.get(node_id)) {
    			highestRank = PR.get(node_id);
    			highestRankNode = node_id;
    		}    		
    	}
		System.out.println("Highest Rank Node: " + highestRankNode);
    	
    	int currentBlock = Integer.parseInt(key.toString());
    	reporter.incrCounter(HighestRankCounter.values()[currentBlock], Long.parseLong(highestRankNode));
    }
}

...
//			System.out.println(sessionID+" "+ version + " " + locationMeta);
			...
			if(request.getCookies().length == 1){//new session
				Server s_local = new Server(InetAddress.getByName(request.getLocalAddr()),request.getLocalPort());
				SID sid = new SID(sessionID, s_local);
			}
...
//			}
			...
			//	System.out.println(InetAddress.getLocalHost().getHostAddress());
				...
				sessionCookie.setMaxAge(0);//remove the cookie
				response.addCookie(sessionCookie);
	...
	private void sessionWrite(SID sid, int new_version, String new_data, Timestamp discard_time){

...
//			System.out.println("cookie length: " + request.getCookies().length);
//			for(int i = 0; i < request.getCookies().length; i++){
//				System.out.println("cookie "+i+ ": "+request.getCookies()[i].getValue());
//			}
...
//			String sessionID = request.getCookies()[0].getValue();
			if(!globalTable.containsKey(sessionID)){
				...
				accessCount = globalTable.get(sessionID).getVersion()+1;
				globalTable.get(sessionID).setVersion(accessCount);
			}
...
//			String value = sessionID+"-"+accessCount+"-"+InetAddress.getLocalHost().getHostAddress();

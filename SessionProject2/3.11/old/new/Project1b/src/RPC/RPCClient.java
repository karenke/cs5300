...
public class RPCClient {
	...
	public static boolean sessionWriteClient(SessionTable session,  Server e) throws IOException{
		DatagramSocket rpcSocket = new DatagramSocket();
		...
		String outStr = callID+"+1+"+session.getSid()+"+"+session.getVersion()+"+"+session.getMessage()+"_"+session.getExpiration().toString();
		outBuf = string2byte(outStr);
	//	for(Server e : s){
			DatagramPacket sendPkt = new DatagramPacket(outBuf, outBuf.length, e.ip, e.port);
			rpcSocket.send(sendPkt);
